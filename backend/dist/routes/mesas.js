...
router.patch('/:id', (req, res) => {
    ...
    init_1.db.run(query, [ocupada ? 1 : 0, id], function (err) {
        if (err) {
        ...
        init_1.db.get('SELECT * FROM mesas WHERE id = ?', [id], (err, row) => {
            if (err) {
...
router.post('/', (req, res) => {
    ...
    init_1.db.run(query, [numero, capacidad], function (err) {
        if (err) {
            console.error('Error al crear mesa:', err);
            return res.status(500).json({ error: 'Error al crear la mesa' });
        }
        // Obtener la mesa creada
        init_1.db.get('SELECT * FROM mesas WHERE id = ?', [this.lastID], (err, row) => {
            if (err) {
...
router.patch('/:id/liberar', (req, res) => {
    const { id } = req.params;
    const query = 'UPDATE mesas SET ocupada = 0 WHERE id = ?';
    init_1.db.run(query, [id], function (err) {
        if (err) {
